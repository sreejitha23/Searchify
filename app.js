...
app.get("/search", (req, res) => {
  ...
  queryKeywords.forEach((key) => {
    ...
    let spellkey1 = spellcheck.getCorrections(key1); //gets similar words
  ...
  */
  ...
  for (let i = 0; i < N; i++) {
    ...
    qid.forEach((key) => {
      ...
      for (let k = 0; k < TF[i].length; k++) {
        if (TF[i][k].id == key) {
          tf = TF[i][k].val / length[i];  //(/length[i] is not needed actually bcz,it is done already while calculating TF[i][k] value)
          break;
        }
      }
      const tfkey = tf;
      ...
      //Custom score to Source reliability
      if (i < 2214) BM25 *= 2;
      s += BM25;
    });

    // Title Similarity
    //Custom score to Title Matching
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const str = path.join(__dirname, "Problems"); //goes to Problems Directory
    const str1 = path.join(str, `problem_text_${arr[i].id + 1}.txt`); //goes to specific problem in Problems directory
